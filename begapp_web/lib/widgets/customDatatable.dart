...
class CustomDataTable extends StatefulWidget {
  ...
  CustomDataTable({
    ...
    this.headerColor: Colors.indigoAccent,
    this.rowColor1: const Color(0xFFE0E0E0),
...
class _CustomDataTableState extends State<CustomDataTable> {
  late BoxDecoration decorationHeader;
  late BoxDecoration decorationRow1;
  late BoxDecoration decorationRow2;
  ...
  Widget build(BuildContext context) {
    return Stack(children: [
      Positioned.fill(
          child: Column(
        children: widget.dataTable.rows.map((e) {
          if (widget.dataTable.rows.indexOf(e) == 0) {
            return Column(children: [
              Container(
                decoration: decorationHeader,
                height: widget.dataTable.headingRowHeight,
              ),
              Container(
                decoration: decorationRow1,
                height: widget.dataTable.dataRowHeight,
              )
            ]);
          } else {
            if (widget.dataTable.rows.indexOf(e) % 2 > 0)
              return Container(
                decoration: decorationRow2,
                height: widget.dataTable.dataRowHeight,
              );
            else
              return Container(
